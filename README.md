...
Где `$ECAM03_SUPERPROJECT` - путь к суперпроекту. Переменная `BUILDROOT_DIR` необходима для получения путей к инструментам Go.
...
# Коприрование фалов.
COPY_FILES+=(
	"init/onvifd.conf|:/etc/onvifd.conf"	"?init/users.digest|:/var/lib/onvifd/users.digest"
...
```bash
# Отключение кеширования фалов
COPY_CACHE=n
# Включение кеширования фалов
COPY_CACHE=y
)
```
...
Список `feature` (фич веб-интерфйса) IP камеры которые должны быть включены или отключены. См. https://docs.elvees.com/pages/viewpage.action?pageId=136127812.
...
5. Периодически после запуска команды `Go: Build Workspace` влкадка «OUTPUT» остаётся пустой, никакого вывода нет. При этом сборка проекта выполняется в фоновом режиме. Это проблема VSCode, проявляется достаточно редко. Обходится повторным запуском `Go: Build Workspace`.
